...
public:
	...
	void reset(){
		while(actualChange != changes.end()){
			...
			(*actualChange).second.actual = (*actualChange).second.initial; 
			actualChange++;
		}
		finished=false;
	}
...
public:
	...
	void reset(){
		while(actualCommand!= commands.end()){
			(*actualCommand).first.reset();
			(*actualCommand).second.actual = (*actualCommand).second.initial; 
			actualCommand++;
		}
		finished=false;
	}
...
public:
	void setup(){
		Box box;
		Command c;
		c.addChange(new RelColorChange(1,-1,1),40, 9000);
		c.addChange(new AbsPosChange(Point(1,2), Point(8,4)),20, 8000);
		/*
		   c.addChange(new RelSizeChange(1,2),40, 8000);
		   c.addChange(new AbsColorChange(Color(1,2,3), Color(40,8,4)),10, 3000);
...
cambiar la box para que tenga changes.
Cambiar los changes por uno solo, con vectores de cambios
De manera que se pueda copiar, nada de punteros ni funciones virtuales: SIMPLE!
